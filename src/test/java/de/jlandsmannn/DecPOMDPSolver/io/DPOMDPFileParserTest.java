...
class DPOMDPFileParserTest {
  ...
  @Test
  void parseObservations_ShouldThrowIfAgentActionsNotInitialized() {
    parser.agentNames = List.of("A1", "A2", "A3");
    var section = "observations:\n" + "5";
    assertThrows(
      IllegalStateException.class,
  ...
  void parseObservationEntry_ShouldThrowIfAgentObservationsNotInitialized() {
    when(builder.getStates()).thenReturn(State.listOf("A", "B"));
    parser.agentActions = List.of(Action.listOf("A", "B"));
    var section = "O: * : * : * : 0.1";
    assertThrows(IllegalStateException.class, () -> {
      parser.parseObservationEntry(section);
    });
  ...
  void parseObservationEntry_ShouldReplaceEndStateWildcardWithAllStates() {
    ...
    when(builder.getStates()).thenReturn(states);
    parser.agentActions = List.of(actions, actions);
    parser.agentObservations = List.of(observations, observations);
